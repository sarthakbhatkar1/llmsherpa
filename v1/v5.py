...
def create_benchmark_chart(data, category):
    """Create a bar chart for a benchmark category"""
    all_metrics = set()
    for model in data:
        benchmark_data = get_benchmark_data(model, category)
        all_metrics.update(benchmark_data.keys())
    ...
    for model in data:
        ...
        for metric in all_metrics:
            ...
            # Convert score to float (remove % if present)
    ...
    fig = px.bar(
        ...
        color="Model",
        barmode="group",
        title=f"{category.replace('_', ' ').title()} Benchmarks",
        labels={"Score": "Score (%)"},
        height=400
    )
    return fig
...
def main():
    ...
    for category_key in filtered_categories:
        ...
        if exists:
            ...
            # Bar Chart
            fig = create_benchmark_chart(filtered_data, category_key)
